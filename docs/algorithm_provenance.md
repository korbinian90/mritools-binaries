...
|---|---|---|---|---|---|
| 1 | Phase rescale to [-π, π] | `MriResearchTools.jl/src/niftihandling.jl` (`readphase`, `rescale=true`) via `src/utility.jl` (`rescale!`) | — | `romeo/main.rs:717-728` (`rescale_phase`) | yes (trivial) |
| 2 | Mask build (`robustmask`/`nomask`/`qualitymask`/FILE) | `MriResearchTools.jl/src/masking.jl` (`robustmask`) | `qsm_core::utils::robust_mask` (quantile rule + morphology) | `romeo/main.rs` `build_mask` → `mritools_common::robust_mask` | no |
| 3 | MCPC-3D-S phase offset correction | `MriResearchTools.jl/src/mcpc3ds.jl` (`mcpc3ds`) | `qsm_core::utils::mcpc3ds_single_coil`, `mcpc3ds_b0_pipeline` | `romeo/main.rs:~260-330` | no |
| 4 | Bipolar correction (if requested) | `MriResearchTools.jl/src/mcpc3ds.jl` eddy-current section | — | delegates to step 3 with `bipolar=true` arg; local fallback in `mcpc3ds/main.rs:273-315` (see mcpc3ds below) | yes |
...
| 6 | Seed selection | `ROMEO.jl/src/seed.jl` (`getseedfunction` / `getseedqueue`) | — | `romeo/main.rs:772-798` (`find_seed`) | yes |
| 7 | Region-growing unwrap | `ROMEO.jl/src/algorithm.jl` (`grow_region_unwrap!`) | `qsm_core::region_grow::grow_region_unwrap` | `romeo/main.rs:693-715` (`unwrap_with_seeds`) | no |
...
| 9 | Quality-map flattening | `ROMEO.jl/src/voxelquality.jl` (`voxelquality`) | — | `romeo/main.rs:800-820` (`compute_quality_map`) | **yes — local algorithm** |
| 10 | B0 fit (phase-weighted least squares) | `MriResearchTools.jl/src/romeofunctions.jl` (`calculateB0_unwrapped`) | — | `romeo/main.rs:340-375` | yes |
...
| # | Step | Julia source | qsm-core function | Rust site | Local? |
|---|---|---|---|---|---|
| 1 | Magnitude combination (`SNR`/`sum-of-squares`/`average`) | `CLEARSWI.jl/src/magnitude_processing.jl` (`combine_echoes_swi`) | — | `clearswi/main.rs:656-737` (`combine_magnitude`) | yes |
| 2 | Sensitivity estimation (N4-style Gaussian smoothing) | `MriResearchTools.jl/src/intensitycorrection.jl` (`getsensitivity`) | `qsm_core::utils::get_sensitivity` | `clearswi/main.rs:~180-245` | no |
| 3 | Bias correction via sensitivity | `CLEARSWI.jl/src/magnitude_processing.jl` (`sensitivity_correction`) | — | `clearswi/main.rs:~245-280` | yes (one divide) |
| 4 | Mask (`robustmask`) | `MriResearchTools.jl/src/masking.jl` (`robustmask`) | `qsm_core::utils::robust_mask` | `clearswi/main.rs` → `mritools_common::robust_mask` | no |
| 5 | Phase rescale | `MriResearchTools.jl/src/niftihandling.jl` (`readphase`) | — | `clearswi/main.rs:776-787` (`rescale_phase`) | yes (trivial) |
| 6 | Phase unwrapping — Laplacian 3-D | `MriResearchTools.jl/src/laplacianunwrapping.jl` (`laplacianunwrap`) | `qsm_core::unwrap::laplacian::laplacian_unwrap` | `clearswi/main.rs:376-418`, 524-535 | no |
...
| 6c | Phase unwrapping — `laplacianslice` | `CLEARSWI.jl/src/phase_processing.jl` (`laplacianslice_combine`) | — | `clearswi/main.rs:528-532` — **stub**, falls back to 3-D laplacian with a warning | see [Stubs](#clearswi-laplacianslice) |
| 7 | High-pass phase filter | `CLEARSWI.jl/src/phase_processing.jl` (`laplacian_combine` / `romeo_combine`, via `gaussiansmooth3d`) | — | `clearswi/main.rs:~420-505` | yes (local 3-D Gaussian) |
| 8 | Multi-echo QSM weighting | `CLEARSWI.jl/src/phase_processing.jl` (`qsm_contrast` / `combine_phase`) | — | `clearswi/main.rs:356-391` | **yes — local algorithm** |
...
| 10 | SWI combine (phase-weighted magnitude + softplus scaling) | `CLEARSWI.jl/src/functions.jl` (`calculateSWI`), `src/magnitude_processing.jl` (`softplus_scaling`), `src/phase_processing.jl` (`createphasemask!`) | `qsm_core::swi::{calculate_swi, softplus_scaling, PhaseScaling}` | `clearswi/main.rs:569-601` | no |
| 11 | MIP | `CLEARSWI.jl/src/functions.jl` (`createMIP` / `createIntensityProjection`) | `qsm_core::swi::create_mip` | `clearswi/main.rs:640-655` | no |
...
| # | Step | Julia source | qsm-core function | Rust site | Local? |
|---|---|---|---|---|---|
| 1 | Phase rescale | `MriResearchTools.jl/src/niftihandling.jl` (`readphase`) | — | `mcpc3ds/main.rs:318-329` (`rescale_phase`) | yes (trivial) |
| 2 | Mask build | `MriResearchTools.jl/src/masking.jl` (`robustmask`) | `qsm_core::utils::robust_mask` | `mcpc3ds/main.rs` → `mritools_common::robust_mask` | no |
...
| # | Step | Julia source | qsm-core function | Rust site | Local? |
|---|---|---|---|---|---|
| 1 | Bias field estimate + correction | `MriResearchTools.jl/src/intensitycorrection.jl` (`makehomogeneous`) | `qsm_core::utils::makehomogeneous` | `makehomogeneous/main.rs:106-114` (3-D), 135-143 (4-D) | no |
...
| # | Step | Julia source | qsm-core function | Rust site | Local? |
|---|---|---|---|---|---|
| 1 | Phase rescale | `MriResearchTools.jl/src/niftihandling.jl` (`readphase`) | — | `romeo_mask/main.rs:347-358` (`rescale_phase`) | yes (trivial) |
| 2 | Initial mask (`robustmask`) | `MriResearchTools.jl/src/masking.jl` (`robustmask`) | `qsm_core::utils::robust_mask` | `romeo_mask/main.rs` → `mritools_common::robust_mask` | no |
...
| 4 | Quality-map flattening | `ROMEO.jl/src/voxelquality.jl` (`voxelquality`) | — | `romeo_mask/main.rs:372-392` (`compute_quality_map`) | **yes — local algorithm** |
| 5 | Threshold + write mask | `ROMEO.jl/ext/RomeoApp/caller.jl` (`set_mask!`, `qualitymask` option) | — | `romeo_mask/main.rs:~225-260` | yes |
...
### romeo-merge-regions
- Flag: `--merge-regions`
- Julia: `ROMEO.jl/src/region_handling.jl` `merge_regions!`
...
- Julia: `ROMEO.jl/src/region_handling.jl` `correct_regions!`
...
- Julia: `CLEARSWI.jl/src/phase_processing.jl` `laplacianslice_combine` slice-wise laplacian path.
